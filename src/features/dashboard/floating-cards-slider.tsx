...
export function FloatingCardsSlider({
  ...
  return (
    <section className="w-full py-6" aria-labelledby={headingId}>
      <h2 id={headingId} className="text-2xl font-bold mb-4 text-center px-4 text-foreground-800">
        {title}
      </h2>
      <div className="w-full px-4 max-w-[1400px] mx-auto">
        <div
          ref={containerRef}
          className="relative overflow-x-hidden overflow-y-visible w-full"
          role="region"
        ...
        >
          {isReady ? (
            ...
            {items.map((route) => (
              ...
              <Link
                to={route.routePath}
                className="block h-full focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-white rounded-lg"
                aria-label={`Przejdź do trasy ${route.name} w ${route.location}`}
              >
                <motion.div
                  className="bg-white rounded-lg shadow-lg overflow-hidden h-full border border-foreground-200 hover:shadow-xl transition-shadow duration-300"
                  style={{
                    boxShadow: '0 4px 6px -1px rgba(33, 150, 243, 0.1), 0 2px 4px -1px rgba(33, 150, 243, 0.06)',
                  }}
                  whileHover={{
                    boxShadow: '0 10px 15px -3px rgba(33, 150, 243, 0.15), 0 4px 6px -2px rgba(33, 150, 243, 0.1)',
                  }}
                >
                  <div className="relative aspect-[4/3] w-full overflow-hidden bg-foreground-100">
                    <img
                      src={route.image}
                      alt={route.name}
                    ...
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/60 to-transparent p-4 text-white">
                      ...
                      <p className="text-sm mb-2 opacity-90">{route.location}</p>
                      {route.description && (
                        <p className="text-xs opacity-75 line-clamp-2">{route.description}</p>
                      )}
                    </div>
                  </div>
                </motion.div>
      ...
      <div
        className="flex justify-end gap-2 mt-4 px-4"
        role="group"
      ...
      >
        ...
        >
          <ChevronLeft className="w-6 h-6 text-foreground-700" aria-hidden="true" />
        </button>
        ...
        >
          <ChevronRight className="w-6 h-6 text-foreground-700" aria-hidden="true" />
        </button>
      </div>
    </section>
  )
